    ...
    override fun getEventMap(): Map<String, (JSONObject) -> Unit> {
        return mapOf(
            "video:sync" to { data ->
                ...
                if (parsed == null) {
                    val error = SocketMessage(type = "receiveVideoSync", message = data.toString())
            ...
            "video:play" to { data ->
                ...
                if (parsed == null) {
                    val error = SocketMessage(type = "receiveVideoPlay", message = data.toString())
                    listener.onError(error)
                }
            ...
            "video:pause" to { data ->
                ...
                if (parsed == null) {
                    val error = SocketMessage(type = "receiveVideoPause", message = data.toString())
                    listener.onError(error)
                }
